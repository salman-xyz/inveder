...
class TransactionForm {
  ...
  createInputGroup(type, index) {
    ...
    if (type === "item") {
      ...
      span.innerHTML =
        '<img src="https://cdn.jsdelivr.net/npm/lucide-static@0.344.0/icons/package.svg" width="16" height="16" alt="Item">';
      ...
      suggestions.className =
        "suggestions d-none position-absolute w-100 bg-white border rounded-bottom shadow-sm";
  ...
  updateUI() {
    this.itemsTitle.textContent = this.state.isTrading
      ? "Trading Items"
      : "Items";
    ...
    this.respectivelyCheck.classList.toggle(
      "d-none",
      this.state.activeItemCount <= 1
    );

    document.getElementById("addItem").disabled =
      this.state.activeItemCount >= 3;
    document.getElementById("removeItem").disabled =
      this.state.activeItemCount <= 1;
  }
  formatPrice(price) {
    if (!price) return "";
    ...
    if (cleanPrice.includes("k")) {
      ...
      if (!isNaN(numericValue)) {
        const formattedValue =
          (numericValue / 1000000) % 1 === 0
            ? (numericValue / 1000000).toFixed(0) // No decimal for whole numbers
            : (numericValue / 1000000).toFixed(1); // Keep one decimal for fractional values
  ...
  formatTransaction() {
    ...
    const validItems = items
      .slice(0, isTrading ? 2 : activeItemCount)
    ...
    if (isTrading) {
      ...
      const formattedQuantity1 = quantity1
        ? parseInt(quantity1).toLocaleString("en-US").replace(/,/g, ".")
        : "";
      const formattedQuantity2 = quantity2
        ? parseInt(quantity2).toLocaleString("en-US").replace(/,/g, ".")
        : "";

      const item1Text = formattedQuantity1
        ? `${formattedQuantity1} ${formattedItem1}`
        : formattedItem1;
      const item2Text = formattedQuantity2
        ? `${formattedQuantity2} ${formattedItem2}`
        : formattedItem2;
    ...
    const formattedItems = validItems
      .map((item, index) => {
        ...
        const formattedQuantity = quantity
          ? parseInt(quantity).toLocaleString("en-US").replace(/,/g, ".")
          : "";
        let formattedItem = formattedQuantity
          ? `${formattedQuantity} ${modifiedItemName}`
          : modifiedItemName;
    ...
    if (validPrices.length > 0) {
      ...
      if (validPrices.length === 1) {
        ...
        output += `. ${
          type === "buying" ? "Budget" : "Price"
        }: ${priceText} respectively`;
      } else {
        output += `. ${type === "buying" ? "Budget" : "Price"}: ${priceText}`;
      }
    } else {
  ...
  updatePreview() {
    ...
    this.preview.textContent =
      output || "Complete the form to generate your transaction description";
  }
...
if (css) css.href = `items.css?v=12${ts}`;

// Update JS link
const js = document.getElementById("mainJs");
if (js) js.src = `items.js?v=12${ts}`;
